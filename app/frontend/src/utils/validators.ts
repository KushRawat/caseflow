...
import type { SchemaFieldId, SchemaMapping } from './schema';
...
export const validateRow = (
  ...
  const errors: Partial<Record<SchemaFieldId, string>> = {};

  const readField = (field: SchemaFieldId) => {
  ...
  if (!applicantName) errors.applicantName = 'Applicant name is required';

  const dob = readField('dob');
  if (!dob) errors.dob = 'Date of birth is required';
  else if (!isValidDate(dob)) errors.dob = 'Date must be between 1900 and today';
  ...
  else if (!categories.has(category)) errors.category = 'Category must be TAX, LICENSE, or PERMIT';
  ...
  const normalized: CaseRowInput = {
    caseId,
    applicantName,
    dob,
    email: email || undefined,
    phone: phone || undefined,
    category: category as CaseRowInput['category'],
    priority: (priority as CaseRowInput['priority']) ?? 'LOW',
    status: 'NEW'
  };
